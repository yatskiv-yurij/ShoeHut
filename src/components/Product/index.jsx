...
export const Product = ({edit, data}) => {
    ...
    const addFavourite = () => {
        ...
        if(favourites){
            ...
            window.localStorage.setItem('favourite', JSON.stringify([data._id]));
        }
    }
    ...
    const link = !edit ? `/shop/${data._id}` : `/account/edit-product/${data._id}`;
    return ( 
         ...
         { data && <>
            ...
            <Link className="product__data" to={link}>
                <p className="product__name product__name_simple">{data.title}</p>
        ...
        {!edit && !favourite && <svg onClick={addFavourite} className='heart empty-heart' width="25" height="23" viewBox="0 0 25 23" fill="none" xmlns="http://www.w3.org/2000/svg">
        ...
        {!edit && favourite && <svg onClick={deleteFavourite} className='heart red-heart' width="25" height="23" viewBox="0 0 25 23" fill="none" xmlns="http://www.w3.org/2000/svg">
